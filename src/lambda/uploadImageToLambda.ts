...
export const uploadImageToLambda = async(event: APIGatewayProxyEventV2) => {
    ...
    if(!data){
        return {statusCode: 400, headers: {}, body: JSON.stringify({response: 'Request does not have a body'})};
    }

    if(!data.image){
        return {statusCode: 400, headers: {}, body: JSON.stringify({response: 'Image property does not exist in request'})};
    }
    ...
    if(!data.image.contents){
        return {statusCode: 400, headers: {}, body: JSON.stringify({response: 'No image content'})};
    }

    // This converts the base64 image into binary data that is accepted by the S3 bucket
    const base64Data = Buffer.from(data.image.contents.replace(/^data:image\/\w+;base64,/, ""), 'base64');
